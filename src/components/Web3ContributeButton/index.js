...
export default function ContributeBtn(props) {
	...
	const [mercuryoFiatCurrencyAmount, setMercuryoFiatCurrencyAmount] = useState(0);
	...
	function getVenlyAuth() {
		...
		axios
			.post(`https://login.arkane.network/auth/realms/Arkane/protocol/openid-connect/token`, formBody, {
				headers: {
	...
	function createVenlyWallet() {
		...
		axios.get(process.env.REACT_APP_BASE_URL + `/api/venly/create_wallet/${venlyEmail}/`)
			.then(res => {
				// console.log("res: ", res.data)
				if(res.data) {
					redirectToMercuryo(res.data.address, res.data.email);
				}
			})
	...
	function redirectToMercuryo(address, email) {
		...
		if(contribution_amount >= 16) {
			...
			window.location.replace(`https://exchange.mercuryo.io/?widget_id=${data.widget_id}&address=${data.address}&signature=${data.signature}&fiat_amount=${data.amount}&type=${data.type}&fiat_currency=${data.from}&currency=${data.to}&email=${data.email}&redirect_url=${data.redirect_url}`);
		} else {
			popupInfo("Donation amount should at least be $16 or above");
		}
	}
